...
async function getIceServers(apiServer: string, apiKey: string, sessionId: string) {
	const response = await fetch(`${apiServer}/api/v1/session/${sessionId}/ice-servers/`, {
		headers: {
			'PersoLive-APIKey': apiKey
		},
		method: 'GET'
...
export const load = async () => {
	...
	const iceServers = await getIceServers(persoLiveApiServerUrl, persoLiveApiKey, sessionId);
